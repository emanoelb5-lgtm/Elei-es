    ...
    private fun parseUncertainty(obj: JSONObject?): UncertaintyData {
        ...
        candidatesObj.keys().forEach { id ->
            ...
            candidates[id] = CandidateUncertainty(
                ...
                parameterStressLow = if (item.isNull("parameterStressLow")) null else item.optDouble("parameterStressLow"),
                parameterStressHigh = if (item.isNull("parameterStressHigh")) null else item.optDouble("parameterStressHigh"),
                parameterStressHalfWidth = item.optDouble("parameterStressHalfWidth", 0.0),
        ...
        return UncertaintyData(
            ...
            parameterStressVariantCount = root.optInt("parameterStressVariantCount", 0),
            parameterStressSensitivity = root.optString("parameterStressSensitivity", "indisponivel"),
            parameterStressAutomaticAdjustment = root.optBoolean("parameterStressAutomaticAdjustment", false),
